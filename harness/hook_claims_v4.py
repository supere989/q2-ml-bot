...
CANDIDATE_SCHEMA = "q2-hook-claim-candidates-v4"
MATERIALIZATION_SCHEMA = "q2-hook-claim-materialization-v4"
...
SELECTED_RECORD_KEYS = CANDIDATE_RECORD_KEYS | {"measured_anchor_milliunits"}
TRACE_KEYS = {
...
class HookClaimsV4Error(RuntimeError):
    """A hook-v4 artifact is malformed, stale, or not canonically encoded."""
...
def _integer(value: Any, label: str, minimum: int | None = None) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise HookClaimsV4Error(f"{label} must be an integer")
    if minimum is not None and value < minimum:
        raise HookClaimsV4Error(f"{label} must be at least {minimum}")
    return value


def _vec3(value: Any, label: str) -> list[int]:
    if not isinstance(value, list) or len(value) != 3:
        raise HookClaimsV4Error(f"{label} must be three integers")
    return [_integer(item, f"{label}[{index}]") for index, item in enumerate(value)]


def _sha(value: Any, label: str) -> str:
    if not isinstance(value, str) or not _SHA_RE.fullmatch(value):
        raise HookClaimsV4Error(f"{label} is not a lowercase SHA-256")
    return value


def _validate_record(
    value: Any, label: str, *, selected: bool,
) -> dict[str, Any]:
    record = _mapping(
        value, SELECTED_RECORD_KEYS if selected else CANDIDATE_RECORD_KEYS, label,
    )
    claim_id = record["claim_id"]
    if not isinstance(claim_id, str) or not _CLAIM_RE.fullmatch(claim_id):
        raise HookClaimsV4Error(f"{label} has invalid claim_id")
    source = _vec3(record["source_milliunits"], f"{label}.source_milliunits")
    trace_target = _vec3(
        record["trace_target_milliunits"], f"{label}.trace_target_milliunits",
    )
    measured_anchor = (
        _vec3(
            record["measured_anchor_milliunits"],
            f"{label}.measured_anchor_milliunits",
        )
    ...
    if release > 49:
        raise HookClaimsV4Error(f"{label}.release_after_ticks exceeds hook lifetime")
    distance = _integer(record["distance_milliunits"], f"{label}.distance_milliunits", 1)
    flags = _integer(record["flags"], f"{label}.flags", 0)
    if flags > 7:
        raise HookClaimsV4Error(f"{label}.flags exceeds 7")
    eye = (source[0], source[1], source[2] + 22_000)
    distance_target = measured_anchor if measured_anchor is not None else trace_target
    expected_distance = round(math.sqrt(sum(
        (distance_target[axis] - eye[axis]) ** 2 for axis in range(3)
    )))
    if distance != expected_distance:
        raise HookClaimsV4Error(f"{label}.distance_milliunits is not source-bound")
    if any(value % 125 for value in source):
        raise HookClaimsV4Error(f"{label}.source_milliunits is not exact Pmove fixed-point")
    normalized = {
        "claim_id": claim_id,
        "source_milliunits": source,
        "trace_target_milliunits": trace_target,
    ...
    if measured_anchor is not None:
        normalized["measured_anchor_milliunits"] = measured_anchor
    return normalized


def validate_candidate_record(value: Any, label: str) -> dict[str, Any]:
    return _validate_record(value, label, selected=False)


def validate_selected_record(value: Any, label: str) -> dict[str, Any]:
    return _validate_record(value, label, selected=True)
...
def validate_candidates(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise HookClaimsV4Error("hook candidate container is not frozen v4")
    values = container["records"]
    if not isinstance(values, list) or not values:
        raise HookClaimsV4Error("hook candidate container has no records")
    records = [
        validate_candidate_record(item, f"candidate {index}")
        for index, item in enumerate(values)
    ]
    ids = [record["claim_id"] for record in records]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise HookClaimsV4Error("hook candidate IDs are not unique and stably ordered")
    return {
...
def load_candidates(meta_path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    try:
        ...
        raise HookClaimsV4Error("generator metadata is not valid JSON") from error
    if not isinstance(metadata, dict):
        raise HookClaimsV4Error("generator metadata is not an object")
    candidates = validate_candidates(metadata.get("hook_claim_candidates_v4"))
    return (
        candidates,
...
def runtime_rows(records: Sequence[Mapping[str, Any]]) -> bytes:
    ...
    for record in records:
        anchor = record["measured_anchor_milliunits"]
...
def selected_records_sha256(records: Sequence[Mapping[str, Any]]) -> str:
    return sha256_bytes(canonical_json([dict(record) for record in records]) + b"\n")


def validation_traces_sha256(traces: Sequence[Mapping[str, Any]]) -> str:
    return sha256_bytes(canonical_json([dict(trace) for trace in traces]) + b"\n")
...
def render_runtime_sidecar(
    ...
    header = [
        "# q2-ml-bot hook zones - compiled exact materialization v4",
        "# schema: q2-hook-runtime-sidecar-v4",
        "# passed: true",
...
def validate_trace(value: Any, label: str) -> dict[str, Any]:
    ...
    if not isinstance(claim_id, str) or not _CLAIM_RE.fullmatch(claim_id):
        raise HookClaimsV4Error(f"{label}.claim_id is invalid")
    frames = trace["origin_fixed_frames"]
    if not isinstance(frames, list) or not frames:
        raise HookClaimsV4Error(f"{label}.origin_fixed_frames is empty")
    normalized_frames = []
    for index, frame in enumerate(frames):
        if not isinstance(frame, list) or len(frame) != 3:
            raise HookClaimsV4Error(f"{label} frame {index} is not fixed3")
    ...
    if grounded != len(normalized_frames) - 1:
        raise HookClaimsV4Error(f"{label} does not end at first grounded frame")
    expected = validation_trace_sha256(claim_id, normalized_frames, grounded)
    if _sha(trace["sha256"], f"{label}.sha256") != expected:
        raise HookClaimsV4Error(f"{label}.sha256 differs")
    return {
        "claim_id": claim_id,
...
def validate_materialization(value: Any) -> dict[str, Any]:
    document = _mapping(value, {
        ...
        "landing_policy", "oracles", "replay", "fresh_strict_replay",
        "request_count",
    }, "hook materialization")
    if document["schema"] != MATERIALIZATION_SCHEMA or document["passed"] is not True:
        raise HookClaimsV4Error("hook materialization is not a passed v4 record")
    if not isinstance(document["map"], str) or not document["map"]:
        raise HookClaimsV4Error("hook materialization map is invalid")
    if document["landing_policy"] != LANDING_POLICY:
        raise HookClaimsV4Error("hook materialization landing policy differs")
    ...
    if not isinstance(selected, list) or len(selected) != RUNTIME_RECORD_COUNT:
        raise HookClaimsV4Error("materialization must select exactly six records")
    records = [
        validate_selected_record(item, f"selected record {index}")
        for index, item in enumerate(selected)
    ]
    ids = [record["claim_id"] for record in records]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise HookClaimsV4Error("selected records are not uniquely and stably ordered")
    geometries = [
        (
            tuple(record["measured_anchor_milliunits"]),
            tuple(record["landing_milliunits"]), record["flags"],
        )
        for record in records
    ]
    if len(set(geometries)) != RUNTIME_RECORD_COUNT:
        raise HookClaimsV4Error("selected runtime geometries are not unique")
    if runtime_records_sha256(records) != document["runtime_records_sha256"]:
        raise HookClaimsV4Error("materialization runtime record digest differs")
    traces_value = document["validation_traces"]
    if not isinstance(traces_value, list) or len(traces_value) != RUNTIME_RECORD_COUNT:
        raise HookClaimsV4Error("materialization must have six validation traces")
    traces = [
    ...
    if [trace["claim_id"] for trace in traces] != ids:
        raise HookClaimsV4Error("validation traces differ from selected records")
    ...
    if seal["schema"] != "q2-b1-runtime-authority-seal-v1":
        raise HookClaimsV4Error("materialization B1 seal schema differs")
    normative = _mapping(
    ...
    if (
        ...
        raise HookClaimsV4Error("materialization B1 seal binding differs")

    strict = _mapping(document["fresh_strict_replay"], {
        "schema", "passed", "record_count", "selected_records_sha256",
        "validation_traces_sha256", "oracles",
    }, "fresh strict replay")
    if strict["schema"] != "q2-hook-fresh-strict-replay-v4" or strict["passed"] is not True:
        raise HookClaimsV4Error("fresh strict replay is not a passed v4 record")
    if _integer(strict["record_count"], "fresh strict replay record count", 1) != RUNTIME_RECORD_COUNT:
        raise HookClaimsV4Error("fresh strict replay must cover exactly six records")
    if _sha(
        strict["selected_records_sha256"], "fresh strict selected records digest",
    ) != selected_records_sha256(records):
        raise HookClaimsV4Error("fresh strict replay selected records differ")
    if _sha(
        strict["validation_traces_sha256"], "fresh strict trace digest",
    ) != validation_traces_sha256(traces):
        raise HookClaimsV4Error("fresh strict replay validation traces differ")
    strict_oracles = _mapping(
        strict["oracles"], {"collision", "pmove", "hook"},
        "fresh strict replay oracles",
    )
    for name in ("collision", "pmove", "hook"):
        strict_oracle = _mapping(strict_oracles[name], {
            "executable_sha256", "tool_identity", "physics_identity", "requests",
        }, f"fresh strict replay {name} oracle")
        for key in ("executable_sha256", "tool_identity", "physics_identity"):
            if _sha(strict_oracle[key], f"fresh strict replay {name}.{key}") != oracles[name][key]:
                raise HookClaimsV4Error(
                    f"fresh strict replay {name} identity differs from discovery"
                )
        _integer(strict_oracle["requests"], f"fresh strict replay {name}.requests", 1)
    ...
    if replay != {
        ...
        raise HookClaimsV4Error("materialization replay versions differ")
    request_count = _integer(
        document["request_count"], "materialization request count", 1,
    )
    expected_requests = sum(
        int(oracles[name]["requests"]) for name in ("collision", "pmove", "hook", "fall")
    ) + sum(
        int(strict_oracles[name]["requests"]) for name in ("collision", "pmove", "hook")
    )
    if request_count != expected_requests:
        raise HookClaimsV4Error("materialization request count differs")
    return document
...
def load_materialization(path: Path) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        raise HookClaimsV4Error("hook materialization is not valid JSON") from error
    document = validate_materialization(value)
    canonical = canonical_json(document) + b"\n"
    if raw != canonical:
        raise HookClaimsV4Error("hook materialization is not canonical JSON plus LF")
