...
EXPECTED_COHORT = "b2g26_final_71431"
...
def _expected_71431_rows() -> list[dict[str, Any]]:
    rows = []
    ordinal = 0
    for style_index, style in enumerate(CONCRETE_STYLES):
        for member in range(4):
            seed = 71_431_000 + style_index * 100 + member
            rows.append({
                "ordinal": ordinal,
...
def _validate_declaration(path: Path) -> tuple[dict[str, Any], str]:
    ...
    _require(declaration["cohort_id"] == EXPECTED_COHORT, "B2 gate accepts only cohort 71431")
    _require(declaration["maps"] == _expected_71431_rows(), "71431 map/seed selection differs")
...
def _validate_dyn_evidence(
    ...
    _require(map_id in {row["map"] for row in declaration["maps"]}, "Dyn map is outside cohort 71431")
