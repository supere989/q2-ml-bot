...
def test_exact_spawn_outranks_closer_shared_floor_sample_deterministically() -> None:
    ...
    assert first_pool == second_pool
    fixed_spawns = {
        (64_000, 64_000, 24_125),
        (320_000, 64_000, 24_125),
    }
    primary_records = [
        record for record in first_pool["records"]
        if record["claim_id"].endswith("candidate:0000")
    ]
    assert len(primary_records) == 3
    assert all(
        tuple(record["source_milliunits"]) in fixed_spawns
        for record in primary_records
    )
    assert all(
        tuple(record["source_milliunits"]) != (192_000, 64_000, 24_125)
        for record in primary_records
    )
